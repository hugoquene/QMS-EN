...
hist(x, breaks=seq(min(x),max(x)+1,1)-0.5, col="grey80", 
     main="", xlab="Waiting time (minutes)", ylab="Number" )
...
> *Example 9.4*: In 2014, the mean income per household in the Netherlands was €34,200.
...
word, see footnote[^fn09-3]). We then mean these times, to
...
```{r centrummaten, echo=FALSE, fig.cap="Histogram of a variable with positively skewed (asymmetric) frequency distribution, with (1) the median, (2) the 10% trimmed mean, (3) the 10% winsorized mean, and (4) the arithmetic mean, indicated. The observed scores are marked along the horizontal axis."}
# centrummaten.R
...
hist(xx, col="grey90", xlab="Score", ylab="Number", main="", 
...
taken in the final year of primary school in the Netherlands.
      ...
      Pupil        Reading  Arithmetic   World Orientation
  ---------------- ------- ------------- ------------------
         1           18       22               55
         2           32       36               55
         3           45       34               38
...
§\@ref(sec:ttoets.onesample) et seq.). 
...
here one deviant answer already brings about a deviation of 
 ...
 is always the result of a division operation, and "with division 
 and multiplication, the rule is that the result must contain just as many
 significant numbers as the measurement value with the smallest number 
 of significant figures." (Translation JW) [^fn09-7] 
...
Table: (#tab:signifigurepercentage) The number of significant figures in the reported
...
§\@ref(sec:significantfigures-mean) applied to division operations.
...
if not the "odds" ($23/6=4$) rounded off to the correct number of significant
figures[^fn09-8].
...
Table: (#tab:centredispersionmeasures) Overview of discussed centre measures and dispersion measures. 
...
new variable, called here `zReading` (note in the parentheses in the first line):
```{r cito-zscores}
zReading <- (cito$Reading-mean(cito$Reading)) / sd(cito$Reading) # z-scores
head(zReading) # first pair of observations of variable zReading
```
...
[^fn09-8]: These "odds" indicate that they are 23 successful students to 6 failed students, i.e., rounded u,p 4 successful students for every failed student.
