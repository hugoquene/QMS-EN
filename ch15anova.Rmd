...
For the version with markers ('met') we see that the cell `met:zwak` scores about equally with the cell `met:gemiddeld` ($p=.99$), and that the cell `met:sterk` scores higher than the cell `met:gemiddeld` ($p=.01$). Hence strong readers perform better than average and/or weak readers, in both text versions. 
...
while there is no difference between `zonder:sterk` and `met:sterk` ($p=.89$). 
