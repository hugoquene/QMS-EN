...
\end{equation}
If we square these residuals and then sum the squares, the result is the $\chi^2$ test statistic (given in Eq.\@ref(eq:chisquared)). If the standardized residual $e>2$ then the observed count or observed frequency in that cell is conspicuously larger than the expected count or frequency (with $\alpha=.05$); if the residual is smaller than $-2$ then the reverse holds. Hence the standardized residuals provide insight in the source of a significant outcome of the $\chi^2$ test. 

For each cell of the frequency table, we can also compute the *adjusted* standardized residual, which takes into account the variation among all residuals (by using row totals $R_i$, column totals $C_j$, and the grand total $N$):
\begin{equation}
  (\#eq:chi2-adjstdresidu)
...
[^fn16-3]: If multiple comparisons are performed, then the critical value of $\alpha$ should be adjusted accordingly, in order to prevent Type I errors. With $k$ comparisons, a safe precaution is to use $\alpha/k$ instead of $\alpha$ (Bonferroni correction). See also §\@ref(sec:anova-oneway-posthoc). 
...
dobbel.chi2.htest$residuals # standardized residuals
dobbel.chi2.htest$stdres # adjusted standardized residuals
```
...
# Titanic.chisq.htest$residuals # standardized residuals
Titanic.chisq.htest$stdres # adjusted standardized residuals
```
