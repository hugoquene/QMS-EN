...
\author{true}
\date{Version compiled 27 Nov 2020}
...
\caption{\label{tab:klankfreq}Frequency distribution 
              of the phonological class of sounds
...
\caption{\label{fig:klankfreq-barplot}Bar chat of the frequency distribution of phonological class of sounds in the Corpus of Spoken Dutch (C=consonant, V=vowel).}
\end{figure}
...
are not used much (the sounds in \emph{ei, ui, au}), and that
...
\CommentTok{\# make barplot from column \textasciigrave{}number\textasciigrave{} in dataset \textasciigrave{}klankfreq\textasciigrave{}}
\KeywordTok{with}\NormalTok{( klankfreq, }\KeywordTok{barplot}\NormalTok{( number, }\DataTypeTok{beside=}\NormalTok{T, }
                          \DataTypeTok{ylab=}\StringTok{"Number"}\NormalTok{,}
                          \DataTypeTok{main=}\StringTok{"Sound frequencies in Dutch (N=2968220)"}\NormalTok{,}
...
\CommentTok{\# or simpler: with(klankfreq, barplot(number) ) \# all defaults }
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{hist}\NormalTok{( troon2012, }
      ...
      \DataTypeTok{xlab=}\StringTok{"Length of pause (s)"}\NormalTok{, }\DataTypeTok{ylab=}\StringTok{"Number"}\NormalTok{, }
...
of significant figures.'' (Translation JW) \footnote{\url{https://nl.wikipedia.org/wiki/Significant_cijfer}}
...
\(P(Z \geq Z^*|H_0)=p\) (blue area),
and it has a probability of \(B\) to have a value in the interval \((-Z^*, +Z^*)\).
