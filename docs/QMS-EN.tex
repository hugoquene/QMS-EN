...
\author{true}
\date{Version compiled 04 Feb 2022}
...
{\centering \includegraphics[width=48.71in]{figures/DrakeBenElHeni2003Fig2} 
...
{\centering \includegraphics[width=6.38in]{figures/correlation} 
...
\(\omega^2\) (``omega squared'') (\citet{MD04}, p.296):
\begin{equation}
  \label{eq:omegasq}
...
\includegraphics[width=15.86in]{figures/DBE2012interaction_v2} \textbackslash caption\{Average comprehension score (along Y axis, with 95\% confidence intervals), for text versions with (met) and without (zonder) structure markers, for weak (zwak), average (gemidd) and strong (sterk) readers (after Van Dooren, Van den Bergh and Evers-Vermeul, 2012).\}\label{fig:DBE12interaction}
...
Weak readers perform worse than average readers, in the text version without markers (\(p<.001\)), but not in the version with markers (\(p=.999\)).
Strong readers perform better than average readers, in the version without markers (\(p<.001\)) as well as with markers (\(p=.013\)).
Text markers have an effect for weak readers (\(p<.001\)) and for average readers (\(p<.001\)) but not for strong readers (\(p=.888\)); hence strong readers do not benefit from structure markers in the text, but weak and average readers do.
Also of interest is the finding that weak readers understand a text with markers as well as strong readers do understand the same text without markers (\(p=.722\)).
\end{quote}
...
For the version with markers (`met') we see that the cell \texttt{met:zwak} scores about equally with the cell \texttt{met:gemiddeld} (\(p=.999\)), and that the cell \texttt{met:sterk} scores higher than the cell \texttt{met:gemiddeld} (\(p=.013\)). Hence strong readers perform better than average and/or weak readers, in both text versions.
...
while there is no difference between \texttt{zonder:sterk} and \texttt{met:sterk} (\(p=.888\)).
Hence strong readers do not benefit from structure markers in the text.
Also of interest is the finding that weak readers understand a text with markers as well as strong readers do understand a text without markers (\(p=.722\)).
...
\footnote{The value found \(\chi^2=12.44\) is slightly under the critical value for 5 d.f. and \(p=.025\), (there \((\chi^2)^*=12.83\)), thus the corresponding probability of this value or a larger value is slightly greater than \(0.025\).}).
...
\toprule
p & 3e-01 & 2e-01 & 7e-02 & 2e-02 & 6e-03 & 1e-03 & 2e-04\\
\midrule
