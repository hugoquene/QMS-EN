...
\author{true}
\date{Version compiled 29 Mar 2021}
...
For each cell of the frequency table, in row \(i\) and column \(j\), we can also compute the standardized residual:
\begin{equation}
  \label{eq:chi2-stdresidu}
  e_{ij} = \frac{(O_{ij}-E_{ij})}{\sqrt{E_{ij}}}
\end{equation}
If we square these residuals and then sum the squares, the result is the \(\chi^2\) test statistic (given in Eq.\eqref{eq:chisquared}). If the standardized residual \(e>2\) then the observed count or observed frequency in that cell is conspicuously larger than the expected count or frequency (with \(\alpha=.05\)); if the residual is smaller than \(-2\) then the reverse holds. Hence the standardized residuals provide insight in the source of a significant outcome of the \(\chi^2\) test.

For each cell of the frequency table, we can also compute the \emph{adjusted} standardized residual, which takes into account the variation among all residuals (by using row totals \(R_i\), column totals \(C_j\), and the grand total \(N\)):
\begin{equation}
  \label{eq:chi2-adjstdresidu}
  e_{ij} = \frac{(O_{ij}-E_{ij})}{\sqrt{E_{ij}\times(1-\frac{R_i}{N})\times(1-\frac{C_j}{N})}}
\end{equation}
The adjusted standardized residuals may also be interpreted as \(Z\) scores, using the critical \(Z\) values given in Appendix \ref{app:criticalZvalues}. Hence the adjusted standardized residuals provide insight in the source of a significant outcome of the \(\chi^2\) test, and they also allow us to assess the contribution of each cell to that outcome\footnote{If multiple comparisons are performed, then the critical value of \(\alpha\) should be adjusted accordingly, in order to prevent Type I errors. With \(k\) comparisons, a safe precaution is to use \(\alpha/k\) instead of \(\alpha\) (Bonferroni correction). See also §\ref{sec:anova-oneway-posthoc}.}.

For the example given in §\ref{sec:chi2gof} we find the following six adjusted standardized residuals for the six possible outcomes of the die: \((1.39, -0.35, +0.35, 0.00, 1.73, -3.12)\). The first five of these outcomes are observed approximately as frequently as expected, but the sixth of these outcomes is observed significantly less often than expected.
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{chisq.test}\NormalTok{( }\KeywordTok{c}\NormalTok{( }\DecValTok{14}\NormalTok{, }\DecValTok{9}\NormalTok{, }\DecValTok{11}\NormalTok{, }\DecValTok{10}\NormalTok{, }\DecValTok{15}\NormalTok{, }\DecValTok{1}\NormalTok{ ) ) {-}\textgreater{}}\StringTok{ }\NormalTok{dobbel.chi2.htest }\CommentTok{\# die §16.2}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{dobbel.chi2.htest}\OperatorTok{$}\NormalTok{residuals }\CommentTok{\# standardized residuals}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{dobbel.chi2.htest}\OperatorTok{$}\NormalTok{stdres }\CommentTok{\# adjusted standardized residuals}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{chisq.test}\NormalTok{( Titanic.classoutcome ) {-}\textgreater{}}\StringTok{ }\NormalTok{Titanic.chisq.htest}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Titanic.chisq.htest$residuals \# standardized residuals}
\NormalTok{Titanic.chisq.htest}\OperatorTok{$}\NormalTok{stdres }\CommentTok{\# adjusted standardized residuals}
\end{Highlighting}
\end{Shaded}
