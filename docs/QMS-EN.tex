...
\author{true}
\date{Version compiled 28 Mar 2021}
...
For each cell of the frequency table, we can also compute the standardized residual:
\begin{equation}
  \label{eq:chi2-residu}
  e = \frac{(O-E)}{\sqrt{E}}
\end{equation}
If we square these residuals and then sum the squares, the result is the \(\chi^2\) test statistic (given in Eq.\eqref{eq:chisquared}). The standardized residuals may also be interpreted as \(Z\) scores, using the critical \(Z\) values given in Appendix \ref{app:criticalZvalues}. If the standardized residual \(e>2\) then the observed count or observed frequency is significantly larger than the expected count or frequency (with \(\alpha=.05\)); if the residual is smaller than \(-2\) then the reverse holds. Hence the standardized residuals provide insight in the source of a significant outcome of the \(\chi^2\) test.

For the example given in §\ref{sec:chi2gof} we find the following six standardized residuals for the six possible outcomes of the die: \((1.26, -0.32, +0.32, 0.00, 1.58, -2.58)\). The first five of these outcomes are observed approximately as frequently as expected, but the sixth of these outcomes is observed significantly less often than expected.
...
In JASP it is not possible to obtain the standardized residuals; however you can compute these manually from the observed and expected counts.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{dobbel.chi2.htest}\OperatorTok{$}\NormalTok{residuals }\CommentTok{\# STANDARDIZED resid in ...$residuals}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Titanic.chisq.htest}\OperatorTok{$}\NormalTok{residuals}
\end{Highlighting}
\end{Shaded}
...
The residuals show the remarkably high number of survivors among the first class passengers, and the remarkably low number of survivors among the ship's crew.
