...
\author{true}
\date{Version compiled 20 Mar 2026}
...
This textbook is being used in the introductory course \emph{Methods and Statistics} at Utrecht University (Linguistics program). The book is also highly suitable for self-study at a basic level, for everybody who wishes to learn more about quantitative methods and statistics.
...
Quené, H. \& Van den Bergh, H. (2024). \emph{Quantitative Methods and Statistics}.
Retrieved 27 Aug 2024 from \url{https://hugoquene.github.io/QMS-EN/} .
...
\item
  ...
  Longer introductions are available in \citet{Dalg02} and in excellent web books; a list of resources is available at
  \url{https://statisticalhorizons.com/resources/free-r-resources}.
\end{itemize}
...
\begin{itemize}
\item
  regarding \emph{individuals}: their height, their weight, shoe size, speaking rate, number of siblings, number of children, political preference, income, sex, popularity within a group, etc.
\item
...
Of course, the opposite is not possible: a variable at a low level of measurement cannot be reinterpreted at a higher level. We would have to add, after the fact, information that we did not collect during the original measurement of this variable. It is therefore imperative to observe the relevant variables at the correct level of measurement. Supposed we wanted to compare body height in adult men and women. If we measure body height at an ordinal level (having defined three categories, \emph{short, medium, and tall}, equally for all individuals), this means that we cannot calculate the average body height, and we can also not use any statistical test that would refer to the average body height. This does not have to be a problem, but it is a good idea to think through the consequences of using a particular level of measurement in advance of the actual measurement.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{enq2011 }\OtherTok{\textless{}{-}} \FunctionTok{read.table}\NormalTok{( }
    \AttributeTok{file=}\FunctionTok{url}\NormalTok{(}\StringTok{"http://www.hugoquene.nl/R/enq2011.txt"}\NormalTok{), }
    \AttributeTok{header=}\ConstantTok{TRUE}\NormalTok{ )}
\FunctionTok{table}\NormalTok{( enq2011}\SpecialCharTok{$}\NormalTok{shoe, }\AttributeTok{useNA=}\StringTok{"ifany"}\NormalTok{ ) }
\end{Highlighting}
\end{Shaded}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{QMS-EN_files/figure-latex/klankfreq-barplot-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{QMS-EN_files/figure-latex/troonrede2012-hist-1.pdf}}
...
\NormalTok{klankfreq }\OtherTok{\textless{}{-}} \FunctionTok{read.table}\NormalTok{( }\AttributeTok{file=}\StringTok{"data/klankfreq.txt"}\NormalTok{, }\AttributeTok{header=}\NormalTok{T )}
\CommentTok{\# 20201130 column names in English}
\FunctionTok{dimnames}\NormalTok{(klankfreq)[[}\DecValTok{2}\NormalTok{]] }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"main.class"}\NormalTok{,}\StringTok{"sub.class"}\NormalTok{,}\StringTok{"count"}\NormalTok{)}
\CommentTok{\# make barplot from column \textasciigrave{}count\textasciigrave{} in dataset \textasciigrave{}klankfreq\textasciigrave{}}
\FunctionTok{with}\NormalTok{( klankfreq, }\FunctionTok{barplot}\NormalTok{( count, }\AttributeTok{beside=}\NormalTok{T, }
                          \AttributeTok{ylab=}\StringTok{"Frequency"}\NormalTok{,}
                          \AttributeTok{main=}\StringTok{"Frequencies of speech sounds in Dutch (N=2968220)"}\NormalTok{,}
                          \AttributeTok{col=}\FunctionTok{ifelse}\NormalTok{(klankfreq[,}\DecValTok{1}\NormalTok{]}\SpecialCharTok{==}\StringTok{"V"}\NormalTok{,}\StringTok{"grey40"}\NormalTok{,}\StringTok{"grey20"}\NormalTok{) ) ) }\OtherTok{{-}\textgreater{}}\NormalTok{ klankfreq\_barplot}
\CommentTok{\# make labels along the bottommost horizontal axis}
\FunctionTok{axis}\NormalTok{(}\AttributeTok{side=}\DecValTok{1}\NormalTok{, }\AttributeTok{at=}\NormalTok{klankfreq\_barplot, }\AttributeTok{labels=}\NormalTok{klankfreq}\SpecialCharTok{$}\NormalTok{main.class)}
\FunctionTok{axis}\NormalTok{(}\AttributeTok{side=}\DecValTok{1}\NormalTok{, }\AttributeTok{at=}\NormalTok{klankfreq\_barplot, }\AttributeTok{tick=}\NormalTok{F, }\AttributeTok{line=}\DecValTok{1}\NormalTok{, }\AttributeTok{labels=}\NormalTok{klankfreq}\SpecialCharTok{$}\NormalTok{sub.class )}
...
\FunctionTok{load}\NormalTok{(}\AttributeTok{file=}\StringTok{"data/pauses6.Rda"}\NormalTok{)}
...
\NormalTok{troon2012 }\OtherTok{\textless{}{-}}\NormalTok{ pauses6[ pauses6}\SpecialCharTok{$}\NormalTok{jaar}\SpecialCharTok{==}\DecValTok{2012}\NormalTok{, }\DecValTok{12}\NormalTok{ ] }\CommentTok{\# save col\_12 as single vector}
\CommentTok{\# make histogram}
\FunctionTok{hist}\NormalTok{( troon2012, }
      \AttributeTok{breaks=}\FunctionTok{seq}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{5}\NormalTok{, }\AttributeTok{by=}\FloatTok{0.25}\NormalTok{),}
      \AttributeTok{col=}\StringTok{"grey80"}\NormalTok{,}
      \AttributeTok{xlab=}\StringTok{"Length of pause (s)"}\NormalTok{, }\AttributeTok{ylab=}\StringTok{"Frequency"}\NormalTok{, }
      \AttributeTok{main=}\StringTok{"Queen\textquotesingle{}s Speech 2012 (N=305 pauses)"}\NormalTok{ ) }\OtherTok{{-}\textgreater{}}\NormalTok{ troonrede2012pauzes\_hist}
\end{Highlighting}
\end{Shaded}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{QMS-EN_files/figure-latex/waittime-hist-1.pdf}}
...
of the sequence of sorted observations \footnote{In American English, the area in the middle of a road (separating traffic in opposite directions) is called the
  ``median (strip)'' (British English: ``central reservation''); this strip typically splits the road into two equally large halves.}. When we sort the observed values of variable \(X\)
...
\begin{quote}
\emph{Example 9.4}: In 2022, the mean income per household in the Netherlands was about €35,000.
The modal income (per household) was between €22,000 and €24,000\footnote{\url{https://www.cbs.nl/nl-nl/visualisaties/inkomensverdeling}}: this income category contained the highest number of households.
\end{quote}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{QMS-EN_files/figure-latex/centraltendencies-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{QMS-EN_files/figure-latex/cito-boxplot-1.pdf}}
...
To create a new variable, click on the \textbf{+} button to the right of the last column name in the data tab. A ``Create Computed Column'' panel appears, where you can enter a name for the new variable, e.g.~\texttt{Lezen\_Z}. You can also choose between \texttt{R} and a hand-shaped pointer. These are the two options in JASP to define formulas with which the new (empty) variable is filled: using \texttt{R} code, or manually using JASP. The paragraphs below explain how standard scores can be computed using these two options. Finally, you can check which measurement level the new variable should be (see Chapter \ref{ch-levelsofmeasurement}). For standard scores, you can leave this at \texttt{Scale}. Next, click on \texttt{Create\ Column} to create the new variable. The new variable (empty column) appears as the rightmost variable in the data set.

If the \textbf{computed with R code} option is chosen to define the new variable, a field containing the text ``Enter your R code here'' appears above the data. Here you can enter R code (see below) that produces standardized values.\\
...
Eventually the definition of standard scores (here for a variable named \texttt{Lezen}) should look like this: \[\frac{(Lezen-mean(Lezen))}{\sigma (Lezen)}\]\\
Fill out the other fields, and then click on the button \texttt{Compute\ column} below the work sheet, to fill the empty variable with the newly computed standard scores.
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{require}\NormalTok{(foreign) }\CommentTok{\# for foreign::read.spss}
\NormalTok{cito }\OtherTok{\textless{}{-}} \FunctionTok{read.spss}\NormalTok{(}\StringTok{"data/cito.sav"}\NormalTok{)}
...
\FunctionTok{fivenum}\NormalTok{(cito}\SpecialCharTok{$}\NormalTok{Lezen) }\CommentTok{\# minimum, Q1, median, Q3, maximum}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{quantile}\NormalTok{(cito}\SpecialCharTok{$}\NormalTok{Lezen, }\FunctionTok{c}\NormalTok{( }\DecValTok{1}\SpecialCharTok{/}\DecValTok{4}\NormalTok{, }\DecValTok{3}\SpecialCharTok{/}\DecValTok{4}\NormalTok{ ) ) }\CommentTok{\# Q1 and Q3, calculated differently }
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{op }\OtherTok{\textless{}{-}} \FunctionTok{par}\NormalTok{(}\AttributeTok{mar=}\FunctionTok{c}\NormalTok{(}\DecValTok{4}\NormalTok{,}\DecValTok{4}\NormalTok{,}\DecValTok{1}\NormalTok{,}\DecValTok{2}\NormalTok{)}\SpecialCharTok{+}\FloatTok{0.1}\NormalTok{) }\CommentTok{\# smaller margins}
\FunctionTok{with}\NormalTok{(cito, }
  \FunctionTok{boxplot}\NormalTok{(Lezen, Rekenen, }\AttributeTok{col=}\StringTok{"grey80"}\NormalTok{, }\AttributeTok{lwd=}\DecValTok{2}\NormalTok{, }\AttributeTok{lty=}\DecValTok{1}\NormalTok{, }\AttributeTok{ylab=}\StringTok{"Score"}\NormalTok{, }\AttributeTok{ylim=}\FunctionTok{c}\NormalTok{(}\DecValTok{17}\NormalTok{,}\DecValTok{45}\NormalTok{) )}
\NormalTok{)}
\FunctionTok{axis}\NormalTok{(}\AttributeTok{side=}\DecValTok{1}\NormalTok{, }\AttributeTok{at=}\FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{,}\DecValTok{2}\NormalTok{), }\AttributeTok{labels=}\FunctionTok{c}\NormalTok{(}\StringTok{"Reading"}\NormalTok{,}\StringTok{"Arithmetic"}\NormalTok{) )}
\NormalTok{plotrix}\SpecialCharTok{::}\FunctionTok{axis.break}\NormalTok{(}\AttributeTok{axis=}\DecValTok{2}\NormalTok{) }\CommentTok{\# break in left Y{-}aixs}
\FunctionTok{rug}\NormalTok{(cito}\SpecialCharTok{$}\NormalTok{Lezen, }\AttributeTok{side=}\DecValTok{2}\NormalTok{) }\CommentTok{\# markings on left Y{-}axis}
\FunctionTok{rug}\NormalTok{(cito}\SpecialCharTok{$}\NormalTok{rekenen, }\AttributeTok{side=}\DecValTok{4}\NormalTok{) }\CommentTok{\# markings on right Y{-}axis}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{QMS-EN_files/figure-latex/cito-summary-boxplot-1.pdf}}

Many \textbf{central tendencies} are pre-programmed as functions in R:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{mean}\NormalTok{(cito}\SpecialCharTok{$}\NormalTok{Lezen) }\CommentTok{\# mean}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{psych}\SpecialCharTok{::}\FunctionTok{winsor.mean}\NormalTok{(cito}\SpecialCharTok{$}\NormalTok{Lezen, }\AttributeTok{trim=}\NormalTok{.}\DecValTok{1}\NormalTok{) }\CommentTok{\# winsorized mean, from psych package}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{mean}\NormalTok{(cito}\SpecialCharTok{$}\NormalTok{Lezen, }\AttributeTok{trim=}\NormalTok{.}\DecValTok{1}\NormalTok{) }\CommentTok{\# trimmed mean}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{median}\NormalTok{(cito}\SpecialCharTok{$}\NormalTok{Lezen)   }\CommentTok{\# median}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{var}\NormalTok{(cito}\SpecialCharTok{$}\NormalTok{Lezen)  }\CommentTok{\# variance}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{sd}\NormalTok{(cito}\SpecialCharTok{$}\NormalTok{Lezen)   }\CommentTok{\# standard deviation, sd(x) = sqrt(var(x))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{mad}\NormalTok{(cito}\SpecialCharTok{$}\NormalTok{Lezen)  }\CommentTok{\# MAD}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{zReading }\OtherTok{\textless{}{-}}\NormalTok{ (cito}\SpecialCharTok{$}\NormalTok{Lezen}\SpecialCharTok{{-}}\FunctionTok{mean}\NormalTok{(cito}\SpecialCharTok{$}\NormalTok{Lezen)) }\SpecialCharTok{/} \FunctionTok{sd}\NormalTok{(cito}\SpecialCharTok{$}\NormalTok{Lezen) }
\FunctionTok{head}\NormalTok{(zReading) }\CommentTok{\# first few observations of variable zReading}
\end{Highlighting}
\end{Shaded}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{QMS-EN_files/figure-latex/binomprobabilitydistribution3-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{QMS-EN_files/figure-latex/binomprobabilitydistribution50n500-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{QMS-EN_files/figure-latex/normalprobabilitydistribution-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{QMS-EN_files/figure-latex/curve.dnorm-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{QMS-EN_files/figure-latex/itunestimeshist-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{QMS-EN_files/figure-latex/itunestimesqqplot-1.pdf}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{itunes }\OtherTok{\textless{}{-}} \FunctionTok{read.table}\NormalTok{( }\AttributeTok{file=}\StringTok{"data/itunestimes20120511.txt"}\NormalTok{, }\AttributeTok{header=}\ConstantTok{TRUE}\NormalTok{ )}
\CommentTok{\# Size in bytes, Time in ms}
\FunctionTok{qqnorm}\NormalTok{(itunes}\SpecialCharTok{$}\NormalTok{Time}\SpecialCharTok{/}\DecValTok{60000}\NormalTok{, }\AttributeTok{datax=}\NormalTok{T, }\AttributeTok{plot.it=}\ConstantTok{FALSE}\NormalTok{) }\CommentTok{\# normally we\textquotesingle{}d use plot.it=TRUE  }
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{shapiro.test}\NormalTok{(itunes}\SpecialCharTok{$}\NormalTok{Time}\SpecialCharTok{/}\DecValTok{60000}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{QMS-EN_files/figure-latex/itunesmeanshist-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{QMS-EN_files/figure-latex/tprobabilitydistribution-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{QMS-EN_files/figure-latex/tempo95CIs-1.pdf}}
...
on the Reading and Arithmetic tests: Pearson \(r=.79, p=.007\). Pupils
...
\endlastfoot
Reading & 1.00 & & \\
Arithmetic & .79 (.007) & 1.00 & \\
Geography & -.51 (.13) - & .01 (.97) & 1.00 \\
\end{longtable}

In this matrix, only the lowest (left) half of the complete
matrix is shown. This suffices because the cells are mirrored
...
always contains the value \(1.00\), since a variable always correlates perfectly with itself.
...
Difference \(v_i\) & -1 & -2 & 2 & 0.5 & -2 & 2 & -1 & 1.5 & -1 & 1 \\
\end{longtable}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{UrbanRural2 }\OtherTok{\textless{}{-}} \FunctionTok{ifelse}\NormalTok{( cito}\SpecialCharTok{$}\NormalTok{Pupil}\SpecialCharTok{\textless{}}\DecValTok{6}\NormalTok{, }\DecValTok{1}\NormalTok{, }\DecValTok{0}\NormalTok{) }\CommentTok{\# 1=urban, 0=rural}
\NormalTok{Arithmetic2 }\OtherTok{\textless{}{-}} \FunctionTok{ifelse}\NormalTok{( cito}\SpecialCharTok{$}\NormalTok{Arithmetic}\SpecialCharTok{\textgreater{}}\FunctionTok{mean}\NormalTok{(cito}\SpecialCharTok{$}\NormalTok{Arithmetic), }\DecValTok{1}\NormalTok{, }\DecValTok{0}\NormalTok{ ) }\CommentTok{\# 1=high, 0=low}
\end{Highlighting}
\end{Shaded}
...
We will illustrate this once more here
with an example. We investigate the statement H1:\\
`Linguistics students master traditional grammar \emph{better} than the average language student'.\\
As a measurement instrument, we use the so-called ``grammar test''\footnote{We thank Els Rose for sharing these data.} which is compulsory
for most students
in language programs at Utrecht University. On the basis of previous cohorts (year groups),
we expect a mean score of 73 on this test;
this is the mean number of correct answers to 100 questions.
We thus operationalise H1 as \(\mu > 73\), and from this deduce
...
For a sample consisting of the first year Linguistics students (\(n=34\)) from a certain cohort,
...
given a random sample of \(n=34\) participants and given that
H0 is in fact true (i.e.~\(\mu=73\)): this probability
\(P=.000000001913\).
This probability \(P\) represents the probability of finding this
...
we may, after all, trust the data obtained.
We are then \emph{not} able to attribute the low \(P\) (probability of the data according to H0) to errors in operationalisation, or to measurement errors, or to other deviations or artefacts in the data.
...
probability \(P\) that rejecting H0 is an incorrect decision (Type I error; see
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{QMS-EN_files/figure-latex/gramm2013onesample-1.pdf}}
...
sample mean (\(n=34\)) if H0 is true. We see that the value 73 has
the highest probability, but also 72 or 74 are probable mean scores
according to H0. However, a mean of 84.4 is very improbable: the probability \(P\) of observing this mean score, or a higher score (area under the curve, to the right of the observed mean) is almost null according to H0.
...
H0. This can be seen from the area under the curve. The coloured part has precisely an area of 0.05 or 5\% of the total area under
the curve.
...
from A to B: a coast path, a mountain path, and a motorway. A walker
...
two choices to choose from the three options. Firstly, the motorway option is discarded (first choice),
and then the mountain path is discarded (second choice),
after which the coast path is the only option remaining.
Thus there are two `free' choices,
in order to choose one of the three possible routes.
If we know the two choices, then we can deduce which
...
degrees of freedom to determine this characteristic of the sample, such as the sample mean here, or such as the test statistic \(t\).
...
We indicate for every test how the degrees of freedom are established
--- and the number of d.f. is usually also calculated and reported by the
statistical analysis program that we use.
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{QMS-EN_files/figure-latex/gramm2013CIs-1.pdf}}
...
The two-sided confidence interval for \(B\)\% reliability for a population mean \(\mu\) is
...
\begin{quote}
Meanwhile, the difference in body height between dwarfs (shorter than
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{QMS-EN_files/figure-latex/smallestsignifdifference-1.pdf}}
...
If we would repeatedly draw many samples from the same population, then the unknown population mean would be within the sample confidence interval of the mean in 95\% of the resamples
(see §\ref{sec:confidenceinterval-mean} and §\ref{sec:t-confidenceinterval-mean}).

This reasoning applies not only to the mean score, or to the
variance, but equally to the effect size as obtained from a single sample.
After all, the effect size too is
an unknown parameter from the population, which we are trying to estimate
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{QMS-EN_files/figure-latex/powercontours-alpha01-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{QMS-EN_files/figure-latex/powercontours-alpha05-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{QMS-EN_files/figure-latex/underpoweredeffectsizes-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{QMS-EN_files/figure-latex/sylduration-boxplot-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{QMS-EN_files/figure-latex/colours-obs-1.pdf}}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{bartlett.test}\NormalTok{( }\AttributeTok{x=}\NormalTok{colourgroups}\SpecialCharTok{$}\NormalTok{score, }\AttributeTok{g=}\NormalTok{colourgroups}\SpecialCharTok{$}\NormalTok{kleur )}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{conmat }\OtherTok{\textless{}{-}} \FunctionTok{matrix}\NormalTok{( }\FunctionTok{c}\NormalTok{(.}\DecValTok{5}\NormalTok{,.}\DecValTok{5}\NormalTok{,}\SpecialCharTok{{-}}\DecValTok{1}\NormalTok{, }\SpecialCharTok{+}\DecValTok{1}\NormalTok{,}\SpecialCharTok{{-}}\DecValTok{1}\NormalTok{,}\DecValTok{0}\NormalTok{), }\AttributeTok{byrow=}\NormalTok{F, }\AttributeTok{nrow=}\DecValTok{3}\NormalTok{ )}
\FunctionTok{dimnames}\NormalTok{(conmat)[[}\DecValTok{2}\NormalTok{]] }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{".R.GB"}\NormalTok{,}\StringTok{".0G.B"}\NormalTok{) }\CommentTok{\# (1) R vs G+B, (2) G vs B }
\FunctionTok{contrasts}\NormalTok{(colourgroups}\SpecialCharTok{$}\NormalTok{kleur) }\OtherTok{\textless{}{-}}\NormalTok{ conmat }\CommentTok{\# assign contrasts to factor}
\FunctionTok{summary}\NormalTok{( }\FunctionTok{aov}\NormalTok{( score}\SpecialCharTok{\textasciitilde{}}\NormalTok{kleur, }\AttributeTok{data=}\NormalTok{colourgroups) }\OtherTok{{-}\textgreater{}}\NormalTok{ m02 ) }
\end{Highlighting}
\end{Shaded}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{figures/DBE2012interaction_v2.png}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{DBE2012 }\OtherTok{\textless{}{-}} \FunctionTok{read.csv}\NormalTok{(}\AttributeTok{file=}\StringTok{"data/DBE2012.csv"}\NormalTok{)}
\CommentTok{\# Versie: 1=with=met, 2=without=zonder structure markers}
\CommentTok{\# Groep: 1=weak=zwak, 2=average=gemiddeld, 3=strong=sterk}
\CommentTok{\# ensure that Versie and Groep are factors, levels are in Dutch here}
\NormalTok{DBE2012}\SpecialCharTok{$}\NormalTok{Versie }\OtherTok{\textless{}{-}} \FunctionTok{factor}\NormalTok{(DBE2012}\SpecialCharTok{$}\NormalTok{Versie, }\AttributeTok{labels=}\FunctionTok{c}\NormalTok{(}\StringTok{"met"}\NormalTok{,}\StringTok{"zonder"}\NormalTok{) ) }
\NormalTok{DBE2012}\SpecialCharTok{$}\NormalTok{Groep }\OtherTok{\textless{}{-}} \FunctionTok{factor}\NormalTok{(DBE2012}\SpecialCharTok{$}\NormalTok{Groep, }\AttributeTok{labels=}\FunctionTok{c}\NormalTok{(}\StringTok{"zwak"}\NormalTok{,}\StringTok{"gemiddeld"}\NormalTok{,}\StringTok{"sterk"}\NormalTok{) )}
\CommentTok{\# DV: MCBiScore: number of comprehension questions (out of 10) }
\CommentTok{\#     that were answered correctly, from a biology text}
\CommentTok{\# make table of means, see Figure 15.3}
\FunctionTok{with}\NormalTok{( DBE2012, }\FunctionTok{tapply}\NormalTok{( MCBiScore, }\FunctionTok{list}\NormalTok{(Versie,Groep), mean ))}
\end{Highlighting}
\end{Shaded}
...
## zonder 2.666667  4.642857 7.571429
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{summary}\NormalTok{( }\FunctionTok{aov}\NormalTok{(MCBiScore}\SpecialCharTok{\textasciitilde{}}\NormalTok{Versie}\SpecialCharTok{*}\NormalTok{Groep, }\AttributeTok{data=}\NormalTok{DBE2012) }\OtherTok{{-}\textgreater{}}\NormalTok{ DBE2012.m1 )}
\end{Highlighting}
\end{Shaded}
...
Open the \texttt{Statistics} section bar, and check the option \texttt{Chi-square} (\(\chi^2\)).
Open the \texttt{Cells} section bar, and check the option \texttt{Expected\ counts}. Also check the \texttt{Pearson\ residuals} and \texttt{Standardized\ (adjusted\ Pearson)}.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{dobbel.chi2.htest}\SpecialCharTok{$}\NormalTok{residuals }\CommentTok{\# raw residuals}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{sum}\NormalTok{( (dobbel.chi2.htest}\SpecialCharTok{$}\NormalTok{residuals)}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{ ) }\CommentTok{\# chi2 = sum of sq of raw resid}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{dobbel.chi2.htest}\SpecialCharTok{$}\NormalTok{stdres }\CommentTok{\# standardized residuals}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Titanic.chisq.htest}\SpecialCharTok{$}\NormalTok{stdres }\CommentTok{\# standardized residuals}
\end{Highlighting}
\end{Shaded}
...
\pandocbounded{\includegraphics[keepaspectratio]{QMS-EN_files/figure-latex/kritiekeZwaarden-hulpfiguur-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{QMS-EN_files/figure-latex/criticaltvalues-auxiliaryfigure-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{QMS-EN_files/figure-latex/criticalchi2values-auxiliaryfigure-1.pdf}}
