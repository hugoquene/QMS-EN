...
\author{true}
\date{Version compiled 06 Mar 2021}
...
From the top bar, choose \texttt{Descriptives}. Select the variable(s) to summarize and place them into the field ``Variables''. Check the option \texttt{Frequency\ tables\ (nominal\ and\ ordinal\ variables)} (below the field with Variables) to obtain frequency tables for all selected variables.
...
This snippet of R code cuts up the continuous variable \texttt{troon12} into the discrete intervals shown in Table \ref{tab:queensspeech2012pauses}:
...
From the top bar, choose \texttt{Descriptives}. Select the variable(s) to summarize and place them into the field ``Variables''. Open the ``Plots'' field and check the option \texttt{Distribution\ plots} (under the header ``Basic plots'').

JASP will automatically make a bar chart if the variable is discrete (nominal or ordinal), and it will make a histogram if the variable is a a scalar, i.e., numerical and continuous (interval or ratio). Ensure that the correct measurement level is specified for your variables. In JASP, you can set the measurement level by clicking on the icon for measurement level, which precedes the variable name in the column header in the data window.
...
From the top bar, choose \texttt{Descriptives}. Select the variable(s) to summarize and place them into the field ``Variables''. Open the ``Plots'' field and check the option \texttt{Distribution\ plots} (under the heading ``Basic plots'') to obtain a histogram or bar chart (depending on the measurement level).
...
From the top bar, choose \texttt{Descriptives}. Select the variable(s) to summarize and place them into the field ``Variables''. Open the ``Statistics'' field and check the option \texttt{Quartiles} (under the heading ``Percentile values'') to obtain quartiles.
...
Here you can examine whether the distribution within each group is normal, which is equivalent to a normal distribution of the \emph{residuals} of the analysis of variance. You can inspect that by checking \texttt{Q-Q\ plot\ of\ residuals}; in the resulting Q-Q plot the residuals should fall on an approximately straight line (see §\ref{sec:isvarnormaldistributed}).
