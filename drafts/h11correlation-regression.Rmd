...
rates
(see also Chapter \@ref(ch:reliability)).
...
arithmetic, and world orientation in the CITO research (see
...
on the Reading and Arithmetic parts: Pearson $r=0.79, p=.007$. Pupils 
with a relatively high score on one part generally also achieve
a relatively high score on the other part.
...
test, as summarised in Table \@ref(tab:cito), with the accompanying significance level between brackets.
                        ...
                        Reading       Arithmetic     World Orientation
  ------------------ -------------- -------------- --------------------
  Reading             1.00                 
  Arithmetic          0.79 (.007)    1.00   
  World Orientation  -0.51 (.131)   -0.01 (.970)         1.00
...
the scores for the Reading and Arithmetic parts: pupils with a relatively
high score on the Reading part generally also achieve a relatively high
...
pupils come from a large city, and the last five from the
countryside. The pupil's place of origin is a nominal variable,
...
to origin (city=1, countryside=0) and according to category of score for the Arithmetic part of 
...
of observations; see text.
  ...
  ---------------- ---------- ---------- ---------------
  Countryside (0)    5 (A)     0 (B)       5 (A+B)
  City (1)           2 (C)     3 (D)       5 (C+D)
...
(`0` and `1`) of the observations. All 5 pupils from the countryside
have an Arithmetic score which is equal to or lower than average
($\overline{y}=$ `r mean(cito$Arithmetic)`); out of the pupils from the city, 2 have a 
...
De dataset `cito` already contains the variable `TownCountry` which indicates the 
...
`TownCountry2`. Indicate that the old values in `Range` from
...
your transformation rules (which relate to `TownCountry`). Then, make a new variable
...
Select the variables `TownCountry2` (in the "Rows" panel) and `Arithmetic2`
...
The dataset `cito` already contains a variable TownCountry which indicates the pupils' origins.
However, for completeness, let us still see how you can construct such a variable for yourself.
```{r phi1}
TownCountry2 <- ifelse( cito$Pupil<6, 1, 0) # 1=town, 0=country
Arithmetic2 <- ifelse( cito$Arithmetic>mean(cito$Arithmetic), 1, 0 ) # 1=high,
0=low
```
But a new variable `Arithmetic2`, which has the value 1 if the score for
Arithmetic is larger than the average, and otherwise has the value 0.
...
``` {r}
print( table(TownCountry2,Arithmetic2) -> citocontingencytable ) 
...
```{r xkcd552, echo=FALSE, fig.cap="_Correlation does not imply causation_, borrowed
with permission from <http://xkcd.com/552>.", fig.align="center"}
