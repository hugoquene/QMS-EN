...
as explained in the Chapter \@ref(ch:hypothesis-testing-research). 
...
§\@ref(sec:normaldistribution) and
§\@ref(sec:whatisnotnormal)). For non-parametric tests, dependent on the technique,
...
§\@ref(sec:falsificaton) and
§\@ref(sec:empiricalcycle).
...
we use the so-called "grammar test"[^fn13-1] which is required for the majority of 
students
in the language domain at Utrecht University. On the basis of previous year groups,
we expect a mean score of 73 on this test; this is the mean number of good answers
from 100 questions. We thus operationalise this first $\mu > 73$, and from this deduce
...
$P=.000000001913$. This probability of $P$ represents the probability of finding this 
...
reliable --- this is precisely the reason why we looked at validity 
(Chapter \@ref(ch:validity)) and reliability 
(Chapter \@ref(ch:reliability)). If we have done everything properly,
we can, after all, trust in the data obtained. 
...
we find this data when H0 is in fact true (in this example: when the linguists
...
dimnames(gramm2013)[[2]] <- c("score","degree course")
# N=233
...
Select the variable `degree course` (drag to the panel on the right-hand side), pick 
button
`=`, and then type *`Linguistics`*, so that the whole condition is
`degree course = Linguistics`.
...
dimnames(gramm2013)[[2]] <- c("score","degree course")
with( gramm2013,
      t.test( score[degree course=="LINGUISTICS"], mu=73, alt="greater" ) )
```
...
to zero. After all, there is always a minimally small probability of finding
an extreme value (or an even extremer value) from you test statistic 
in a sample --- after all, we are investigating the sample
...
The p-value or significance is after all not equal to zero, but has
...
## One-sample and two-sample tests {#sec:ttest-onesidedtwosided}
...
difference, then a complication arises. After all, both differences in one direction
and the other direction are relevant. We speak then of two-sample tests.   
...
coloured regions, on both sides of
the probability distribution).

In the grammar test example, let us now use a two-sample test. We
...
the critical value $t^*=2.042$. The two-sample p-value		
for this critical value is $2 \times .025 = .05$. The test statistic we found
$t=7.9$ is larger than this $t^*=2.042$, thus the two-sample 
p-value is smaller than $p=2\times.025=.05$. The test statistic we found 
is larger even than $t^*=3.385$ in the last column,
thus the two-sample p-value is even smaller than 
$2\times.001$. We can report our two-sample testing as 
follows:
...
In the majority of studies two-sample tests are used; if the direction
of the test is not stated then you can assume that two-sample tests
have been used. 
...
§\@ref(sec:confidnceinterval-mean), and illustrates the confidence interval
...
with $\mu=84.4$ and $\sigma=8.4$ (see
§\@ref(sec:standarddeviation)) and $N=34$. For each sample, we have drawn the 95%
confidence interval. For 95 of the 100 samples, the population average $\mu=84.4$
is indeed within the interval, but for 5 of the 100 samples it is incorrectly not within
the interval (these are marked along the right hand side).
...
output.
```{r}
with( gramm2013, t.test( score[degree course=="LINGUISTICS"] ) )
```
...
$\overline{x_2}=85$ words ($s_2=98$). With this data, we determine the 
...
is expressed here as the mean length of a spoken syllable,
...
$n=20$) that is $\overline{x_N}=0.269$ s ($s=0.029$). With this data,
...
are around equal. SPSS solves this very clumsily: you get to see all the
...
require(hqmisc)
data(talkers)
with(talkers, t.test( syllen[region=="W"], syllen[region=="N"], 
...
Dutch second person pronouns *U* (you *formal*) and *je* (you *informal*) as forms 
of address on a website. The researcher makes two versions
...
variable score of the $i$-th respondent for condition 1. The 
difference scores are also listed in 
...
* The *difference scores* $D$ have to be normally distributed (see		
§\@ref(sec:isvarnormallydistributed)); however, if the number of pairs of
...
> *Example 13.2:*
In an investigation of the life expectancy of dead 50+-ers from Austria
and Denmark [@Dobl99], it appears that life expectancy differs according to
the week of birth, presumably because babies from "summer pregnancies"
are (or were) on average somewhat healthier than those
from "winter pregnancies". In this investigation, the differences
in life expectancy were very small $\pm 0.30$ year in Austria, $\pm 0.15$ year
in Denmark), but the number of observations (dead people) was very large.
...
gramm2013 <- read.csv( file="data/grammaticatoets2013.csv",header=F)
dimnames(gramm2013)[[2]] <- c("score","degree course")
with(gramm2013, score[degree course=="LINGUISTICS"]) -> score.LINGUISTICS 
# auxiliary variable
( mean(score.LINGUISTICS)-73 ) / sd(score.LINGUISTICS) 
```
...
that $r=.9135$. With this data, we find $\textrm{se}_d = 0.519$ via
...
But: we now know not only *that* the preference deviates from zero, but
...
[^fn13-1]: We would like to thank Els Rose for making this data available.
