...
This Chapter is about a much used statistical analysis, called
analysis of variance (often abbreviated to ANOVA). 

How is this chapter put together? We will begin in Section
\@ref(sec:anova-examples) with some examples of research studies whose outcomes
...
when we want to analyse data of a factorial design
(§\@ref(sec:design-factorial)).
...
length of a syllable, with the mean taken over an interview of approximately
15 minutes with each speaker [@Quene08] [@R-hqmisc]. A shorter mean
syllable duration thus corresponds to a quicker speaker (cf. skating: a shorter
lap time corresponds to a quicker skater). There were 20 
...
in Table \@ref(tab:sylduration) and
Figure \@ref(fig:sylduration-boxplot).
...
data(talkers)
with( subset(talkers,syllen<0.4), 
      boxplot( syllen~region, col="grey90",
...
speech tempo, we could in the first instance conduct *t*-tests for all pairs
of levels. (With 4 levels, that would require 6 tests, see
comparison \@ref(eq:choose) with $n=4$ and $x=2$). There are however various
objections to this approach. We will discuss one of these here. For
each, we use a p-value of $\alpha=.05$. 
...
comparison \@ref(eq:probability-productrule)).
...
maintainability of the aforementioned null hypothesis on the basis of 
...
if you a priori are not able or do not want to make any specific predictions
...
like health, wealth, or education. We would only be able to exclude these other
...
whether (ii) the two genders differ from each other (H0:
$\mu_\textrm{woman} = \mu_\textrm{man}$), and whether (iii) the differences
...
regions). We call the latter the 'interaction' between the two factors. 

Table: (#tab:sylduration2way) Mean speech tempos, with standard deviation and 
numbers
of speakers, divided according to gender and the speaker's region of origin
(see Example 15.1).
...
§\@ref(sec:isvarnormallydistributed)).
...
(dashed lines in
Figure \@ref(fig:colours-obs)) relative to the mean over
...
$s^2_x$ (see the identical comparisons
\@ref(eq:variance) and
\@ref(eq:MStotal)).
...
Table: (#tab:colours-anova) Summary of analysis of variance of the observations in Figure \@ref(fig:colours-anova).
...
generalisation to the population then to the sample, we prefer $\omega^2$
...
scores from the red, black and blue groups. The null hypothesis which was tested
was H0:
...
contrasts like the two above (see
Table \@ref(tab:colours-anova-contrast)). We omit further explanation
about planned comparisons but our advice is, where possible, to make 
smart use of these planned comparisons, if you can formulate a more
specific null hypothesis then H0: "the mean scores are equal 
in all groups or conditions". As such, we can make planned statements
...
red group achieves significantly lower scores than the
...
The analysis of variance with planned comparisons can thus be used in particular,
if you already have planned hypotheses over differences between
certain (combinations of) groups or conditions, before the
...
$0 -0.5 + 0.5 = 0$. If the sum of these products is null, then the two
...
i.e., the null hypotheses are already not neutral. 
...
which they make about the variances in the cells. Here, we will mention one test
...
We will use the data in the file `data/kleurgroepen.txt`; this data is also shown in Figure \@ref(fig:colours-obs).
...
"Dependent variable").\
Select `colour` as independent variable (drag to the panel "Fixed
Factor(s)").\
...
the form of ` Partial eta square`. It is, however, better to report
$\omega^2$ but you do have to then calculate it yourself!

#### planned comparison

For an analysis of variance with planned comparisons, we have to 
indicate the desired contrasts for the factor `colour`. However, the method
...
account the *alphabetic* order of the conditions: blue, grey,
red.\
...
Select the button `Post Hoc...`, and select the factor `colour`
(move to the window "Post Hoc Tests for:"). Tick: `Tukey`, and
then `Continue`. Confirm all options with `OK`.
...
```{r}
bartlett.test( x=colourgroups$score, g=colourgroups$colour )
```

#### ANOVA

```{r}
summary( aov( score~colour, data=colourgroups) -> m01 ) # see Table 15.3
```
...
omegasq( m01, "colour" ) # call function with 2 arguments
```
...
contrasts(colourgroups$colour) <- conmat # assign contrasts to factor
summary( aov( score~colour, data=colourgroups) -> m02 ) 
...
of the difference. If that interval does *not* include null, then
...
Figure \@ref(fig:drakebenelheni2003fig2) (§\@ref(sec:design-factorial)).
...
on the other hand is indeed significant. The effect of one factor is precisely
contrasted in the different levels of another factor.
...
extended in Table \@ref(tab:syllen2way-anova). We now test and report three 
...
[$F(3,71)=6.0, p=.0010, \omega^2=.14$], and men speak significantly more quickly
than women [$F(1,71)=15.1, p=.0002, \omega^2=.13$]. 
The two 
factors show no interaction
[$F(3,71)=1.4, \textrm{n.s.}, \omega^2=.01$]. 
A
post hoc comparison between the regions showed that speakers from
...
Table: (#tab:syllen2way-anova) Summary of two-way analysis of variance of the speech
...
Data > Select cases...
```
Indicate that we are only using observations which satisfy a condition,
and indicate that that condition is called `syllen < 0.4`.
...
Analyze > General Linear Model > Univariate...
```
Drag the dependent variable (`syllen`) to the box Dependent
...
require(hqmisc) # for hqmisc::talkers data set
data(talkers)
ok <- talkers$syllen<0.4 # TRUE for 79 of 80 talkers
# table of means, see Table 15.2 in main text
with(talkers, tapply( syllen[ok], list(region[ok],sex[ok]), mean ))
```
...
summary( aov(syllen~region*sex, data=talkers, subset=ok) -> m03 )
```

#### Effect size {#effectsize}

For this, we will use the previously programmed function `omegasq` (§\@ref(R:
squared)): 

```{r}
omegasq(m03, "region")      # [1] 0.1380875
omegasq(m03, "gender")         # [1] 0.1291232
