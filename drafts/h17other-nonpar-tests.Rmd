...
a much used nonparametric test, the $\chi^2$-test. Below, we will look at
a number of other much used nonparametric tests. We discuss these in two groups:
...
which use information of nominal level (sign tests and related) and
then the tests which use information of ordinal level, i.e. which are based on the
...
$p=1/2$. We now determine the probability of this found outcome (0 hits)
given H0, and we use the binomial probability distribution for this
(§\@ref(sec:binomialdistribution)):
\begin{equation}
  (\#eq:prob-binom-uje)
...
outcome, we decide to reject H0, and we report this as 
follows: 
...
observations (§\@ref(sec:test-paired)).

This test makes use of the *order* of the difference $D$ between
the two paired observations. We will again use the example of the imaginary
study on webpages with *U* or *je* as forms of
address
(Table \@ref(tab:data-uje-paired)), but will now look at the *order* 
...
$|W_-|$. 
Here, we will not discuss the probability distribution of the test statistic
...
If all the lowest rankings are in the smallest (first) condition
(i.e. all lowest judgements), then $W_1$ has the minimal value of
$n_1 (n_1+1) /2$. If all the highest rankings are in this condition
(i.e. all the highest judgements), then $W_1$ has the maximum value of
$n_1 (n_1+n_2+1) / 2$. $W_1$ (and the minimum and maximum of it) can only
be integers. 
...
\end{equation}
With this, we again determine the effect size
by comparison \@ref(eq:es-z2r). 
For the above example, we thus find 
...
§(sec:wilcoxon-rank-sum) above), for $k \ge 2$ independent samples
...
and $n_j\ge5$ for the smallest group [@Ferg89]; if  
$k=2$ or $n_j<5$ then the probability $P(H)$ is calculated precisely.
...
than in condition 1), then we can halve the calculated $p$, since the calculated
$p$ assumes two-sided testing. We would thus then find $p=.0653/2=.0326$, and, 
since this probability is smaller than $\alpha=.05$, we would then indeed
